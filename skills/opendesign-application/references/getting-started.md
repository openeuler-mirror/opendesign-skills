...
pnpm add -D sass-embedded           # SCSS 支持（Vite SPA 必装）
```
...
| `@vueuse/core` / `@vueuse/nuxt` | `useScreen()` 等运行时响应式工具 | 主题检测依赖 |
| `sass-embedded` | 编译 `<style lang="scss">` 与 mixin | 必需 |
...
export default defineNuxtConfig({
  ...
  vite: {
    optimizeDeps: { include: ['@opensig/opendesign', '@vueuse/core'] },
    css: {
      preprocessorOptions: {
        scss: {
          additionalData: `
