...
**用途**：新建或改造项目采用 OpenDesign 设计系统时加载此 skill。覆盖主题集成、全局样式基础设施、工程形态选型与 Code Review 检视。
...
| 脚手架 | 形态 | 适用场景 | 参考 |
|--------|------|---------|------|
| Nuxt 4 SSR | 服务端渲染 + 静态生成 | 官网、内容站、需要 SEO | [`templates/nuxt`](skills/opendesign-application/templates/nuxt) |
| Vite + Vue 3 SPA | 单页应用 | 后台管理、工具站、不需 SSR | [`templates/vue-spa`](skills/opendesign-application/templates/vue-spa) |
...
**与其他 skill 的关系**：
- 不重复组件 API（→ opendesign-components）、token 表（→ opendesign-tokens）、设计稿生产（→ opendesign-design）、设计师侧代码直出（→ opendesign-codegen）、CLI 命令（→ opendesign-scripts）
- 专门解决"如何把 OpenDesign 设计系统正确集成进工程项目"

**目录结构**：
```
skills/opendesign-application/
├── SKILL.md                          # 工程化总纲 + 红线 + 起手式 + 参考文件索引
├── references/
│   ├── getting-started.md            # 起手
│   ├── theme-system.md               # 主题系统集成
│   ├── styles-infrastructure.md      # 样式基础设施
│   ├── project-layout.md             # 项目布局
│   └── conventions.md                # 编码约定 + Code Review
└── templates/
    ├── nuxt/                          # Nuxt 4 SSR 脚手架（含项目级 AGENTS.md）
    └── vue-spa/                       # Vite + Vue 3 SPA 脚手架（含项目级 AGENTS.md）
```
...
2. clone 对应脚手架（Nuxt 或 Vite SPA）作为起点（参考 **opendesign-application**）
3. 选择主题，引入 token CSS（参考 **opendesign-tokens**）
4. 集成主题系统与样式基础设施（参考 **opendesign-application**）
5. 使用组件搭建页面（参考 **opendesign-components**）
6. 如有自定义图标，使用 `gen:icon`（参考 **opendesign-scripts**）
