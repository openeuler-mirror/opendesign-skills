...
description: 生成、更新、自评 OpenDesign 生态的 Skill 文件（Vue 组件代码 Skill、CLI 脚本 Skill、Pixso 设计规范 Skill、设计模式 Skill）。当用户要为新组件/脚本/设计规范编写 Skill、更新已有 Skill、做 Skill 自评、追踪生成进度，或询问"如何生成 Skill""Skill 文件格式""自评怎么做""Pixso MCP 调用顺序""Playwright 快照怎么用""componentKey 怎么查""变体维度怎么提取"时，使用本 skill。无论用户是否明确提到"Skill"这个词，只要涉及为 OpenDesign 组件库产出/更新/评审 Skill 知识文件，都应使用本 skill。
---
...
| 关心什么 | 去读哪份 reference |
|---|---|
| 写 / 更新 **代码侧** Skill（Vue 组件 Props/Events/Slots、CLI 脚本命令） | [`references/code-skill-gen.md`](references/code-skill-gen.md) |
...
输出到 `skills/opendesign-{components,scripts}/references/{name}.md`。
...
本 skill 的变更随根 `CHANGELOG.md` 统一维护。变更记录维护规范见仓库根 `AGENTS.md` 的「变更记录规范（CHANGELOG）」章节。
